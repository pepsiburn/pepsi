...
/**
* Licensed under the Electronic Data Systems Corporation
* End User License Agreement
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
*      http://itpos201/adfgen/license.html
*/

/*$RCSfile: validateExactNumber.js,v $ $Revision: 1.1 $ $Date: 2010/12/02 09:31:13 $ */
/**
...
function validateExactNumber(form) {
    var isValid = true;
    var focusField = null;
    var i = 0;
    var fields = new Array();
    ...
    oNumber = eval('new ' + formName.value + '_exactNumber()');
    ...
    for (x in oNumber) {
		...
		if (field != undefined && field != null) {
	        if (( field.type == 'text' &&
	             (field.getAttribute("stype") == "number" || field.getAttribute("stype") == "currency")) &&
	              field.disabled == false && field.value != "") {
	
	            //var value = parseFloat(field.value);
	            var valueStr = normalizeNumber(field.value);
	            valueStr = valueStr.replace(new RegExp( "(\\+|\\-)", "g" ), "");
	            var value = parseFloat(valueStr);
	            jslog.debug("current field:" + field.name + " value:" + field.value + " parsed to float:" + value);
	            if (isNaN(value)) {
	                if (i == 0) {
	                    focusField = field;
	                }
	                ...
	                var numInteri = valueStr.indexOf(".");
	                var numDecimali = 0;
	                if (numInteri == -1) {
	                    numInteri = valueStr.length;
	                } else {
	                    numDecimali = valueStr.length - numInteri - 1;
	                }
	                var intDigit = parseInt(oNumber[x][2]("intDigit"));
	                var decDigit = parseInt(oNumber[x][2]("decDigit"));
	
	                jslog.debug("Num Interi:" + numInteri + " - Num Decimali:" + numDecimali);
	
	                if (numDecimali > decDigit || numInteri > intDigit) {
	                    if (i == 0) {
	                        focusField = field;
	                    }
	                    fields[i++] = oNumber[x][1];
...
/*$RCSfile: validateNumber.js,v $ $Revision: 1.1 $ $Date: 2010/12/02 09:31:13 $ */
/**
*  Check to see if fields must contain a value.
* Fields are not checked if they are disabled.
* <p>
...
function validateNumber(form) {
    ...
    oNumber = eval('new ' + formName.value + '_number()');
    ...
    for (x in oNumber) {
		...
		if (field != undefined && field != null) {
	        if (( field.type == 'text' &&
	            ...
	            var value = normalizeNumber(field.value);
	            value = value.replace(new RegExp( "(\\+|\\-)", "g" ), "");
	
	            jslog.debug("current field:" + field.name + " value:" + field.value + " normalized to: " + value + " and parsed to float:" + parseFloat(value));
...
/*$RCSfile: validateDate.js,v $ $Revision: 1.1 $ $Date: 2010/12/02 09:31:13 $ */
/**
...
function validateDate(form) {
    ...
    for (x in oDate) {
		...
		if (field != undefined && field != null) {
	        ...
	        if (!isValidDateField(field, datePattern)) {
                if (i == 0) {
                    focusField = field;
                }
                fields[i++] = oDate[x][1];
...
function isValidDateField(field, datePattern) {
	if (!datePattern) {
    ...
    if ((field.type == 'hidden' ||
         ...
         field.type == 'textarea') &&
         value.length > 0 &&
         field.disabled == false && value != DATE_FIELD_DEFAULT_VALUE) 
    {
        var MONTH = "MM";
        var DAY = "dd";
        var YEAR = "yyyy";
...
/*$RCSfile: validateRequired.js,v $ $Revision: 1.1 $ $Date: 2010/12/02 09:31:13 $ */
/**
*  Check to see if fields must contain a value.
* Fields are not checked if they are disabled.
* <p>
    ...
    for (x in oRequired) {
		...
		if (field != undefined && field != null) {
	        if ((field.type == 'hidden' ||
	            ...
	            if (trim(value).length == 0 || value == '__/__/____') {
